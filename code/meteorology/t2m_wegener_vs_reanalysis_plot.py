...
for iplot, station in enumerate(('AT_Qameruj', 'AT_Weststation')):
    ...
    x = sub['date']
    plt.plot(x, sub['cera20c'], c=colors[0], label='cera20c')
    plt.fill_between(x, sub['cera20c'] - sub['cera20c_spread'], sub['cera20c'] + sub['cera20c_spread'],
                     alpha=0.6,
                     color=colors[0])
    plt.plot(x, sub['ncep'], c=colors[1], label='ncep')
...
for iplot, station in enumerate(('AT_Qameruj', 'AT_Weststation')):
    ...
    sub = df[df['station'] == station]
    r2_cera20c = sub.corr(numeric_only=True)['cera20c']['wegener'] ** 2
    r2_ncep = sub.corr(numeric_only=True)['ncep']['wegener'] ** 2
    # remove all nans
    ...
    plt.errorbar(nona['wegener'], nona['ncep'], xerr=nona['ncep_spread'], fmt="x", elinewidth=1, markersize=4,
                 color=colors[1],
                 alpha=0.8, label=f'ncep $r^2={r2_ncep:.2f}$')
    sns.despine()
